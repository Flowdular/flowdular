...
14. A screen never writes a bare `select.ui-select`, a bare date input, or its
    own label for either: it uses `Select`, `DateField`, or `DateRangeField`,
    which own the label association, the invalid state, and the help or error
    line. The only bare `ui-select` left is the one a primitive renders inside
    itself (the `Pagination` page size), where the row is the label and the
...
that needs the same arithmetic elsewhere. Default page size is 25.
...
| `Table`            | The one data table, backed by `@octanejs/tanstack-table`: `columns` (`key`, `header`, required `width`, `numeric`, `value`, `cell`), `rows`, `rowKey`, `status` idle/loading, `loadingLabel`, `empty` and `emptyFiltered` picked by `filtered`, `sorting` with `sortingState` and `onSortingChange`, `globalFilter`, `pagination`, `actions(row): TableAction[]`, `actionsLabel`, optional stable `actionsWidth` (160 px by default, 280 px for two actions), `onSelect` with `selectedKey`, `caption`; fixed layout and `colgroup` keep columns stable across states |
...
| `Pagination`       | The pager for the `TableCard` `after` slot: `pageIndex`, `pageSize`, `totalRows`, `onPageChange`, `pageSizes` with `onPageSizeChange` (the type couples the pair, so one cannot arrive without the other), `label`, `previousLabel`, `nextLabel`, `pageSizeLabel`, and `summary(range)` that the screen translates                                                                                                                                                                                                                                                    |
| `Select`           | The labelled native select: required `id`, `label`, `options` (`value`, `label`, `disabled`), `value`, `onChange`, `placeholder`, `name` (defaults to `id`), `required`, `disabled`, `invalid`, `help`, `error`                                                                                                                                                                                                                                                                                                                                                       |
...
and `DateRangeField` are loading when the screen disables them, empty with an
empty `value` (and a `placeholder` on `Select`), in error through `error` or
`invalid`, populated with a value, and denied by not being rendered at all.
...
(+`__reading`), `ui-daterange` (+`__row`), `ui-tabs` (+`__tab`, `__tab--on`),
