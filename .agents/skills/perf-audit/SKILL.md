...
- The repository constructor checks the migration ledger on every open and applies only pending migrations. Runtimes still create the repository once (`service ??= ...` in `src/server/runtime.ts`); constructing one per request adds synchronous ledger reads and connection setup to every request.
...
- Bounds that exist: `maxSteps` 1 to 32 and `timeoutMs` 250 to 86400000 per agent definition (`modules/agents/src/services/agent-service.ts`), worker concurrency `CL_AGENT_WORKER_CONCURRENCY` (1 to 16, default 2) and lease `CL_AGENT_WORKER_LEASE_MS` (`modules/agents/src/server/runtime.ts`).
