---
name: cli-extension
description: Add a module-owned CLI command through commands.json and defineCliExtension, with the namespace, risk, approval, and dry-run rules the runner enforces.
roles:
  - backend-engineer
  - module-executor
  - reviewer
when: A module needs an operator command (status, export, grant, verify) reachable as pnpm coreloom <namespace> <action>.
---
...
- `moduleId` equals `module.json` `id`; the module must be enabled in `coreloom.json`, otherwise its commands do not load.
...
- `localOnly: true`: refused with `LOCAL_ONLY_CAPABILITY` unless `CL_ENV` or `NODE_ENV` is `development` or `test` (unset counts as development).
...
- Invocation: `pnpm coreloom inventory export --apply` or `pnpm coreloom capability run inventory.export --apply`; `arguments` are the positionals after the path (or after `capability run <id>`); flags are `--name value`, `--name=value`, or `--flag` (`packages/cli/src/arguments.ts`). `pnpm coreloom capability list` and `describe <id>` show the descriptor; `pnpm coreloom help` lists module paths.

## 5. Tests

`packages/cli/tests/extensions.test.ts` shows the style: call `validateCliCatalog(catalog, manifest)` with a good catalog and with a path that claims a reserved group, assert the error text. In the module, test `execute` directly with a hand-built context (`apply: false` returns the dry-run shape, `apply: true` writes inside `context.workspaceRoot` only). Then `pnpm coreloom module validate --json` and `pnpm coreloom help` (the new path appears once the module is enabled).
