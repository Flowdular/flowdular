...
CLI: commands are dispatched in `packages/cli/src/runner.ts`; a new core capability is a descriptor in `packages/cli/src/capabilities.ts` (`id`, `version`, `summary`, `risk`, `requiresApprovedSpec`, `supportsDryRun`); flags are parsed by `packages/cli/src/arguments.ts` (`--name value` or `--name=value`, `--flag`); `reservedGroups` in `packages/cli/src/extensions.ts` protects core groups from module namespaces; `pnpm oerp help` output lists the commands. Tests in `packages/cli/tests`. Update `.ai/policies/capabilities.yaml` and `README.md`.
...
The agent-tool hook and module settings show the shape of a safe contract change (`modules/auth/src/server/composition.ts`, `packages/kernel/src/{tool-registry,module-settings}.ts`, `platform/octane.config.ts`):
...
New context members are required (every module receives them; nobody has to read them), new composition members are optional (existing modules compile unchanged). The platform composes all modules, declares every `settings`, then calls every `start`, so `agents.core` reads `context.agentTools.list()` inside `start` and module order stops mattering. The generic registry lives in `@coreloom/kernel` so `modules/auth` does not import the harness; the tool type is applied by `agents.core`. Consumers migrated in the same change: `packages/cli/src/module-sync.ts` (unchanged generator), `modules/agents/src/platform.ts`, `packages/sandbox/src/server/preview-runtime.ts` (builds the same context for a draft), the scaffold templates, `docs/adr`, and the skills that describe the surface (`agent-tool-design`, `module-new`, `module-update`).

## 4. Generated and composed files

`platform/src/generated/modules.server.ts` and `modules.client.ts` are written by `pnpm oerp module sync --apply` (also run by `pnpm dev` and `pnpm build`). `coreloom.json` `modules.enabled` and `platform/package.json` dependencies are written by `module enable --apply`. Never edit them by hand; change the generator and regenerate. `packages/ui/src/brand/mark.ts` is generated by `node packages/ui/scripts/gen-mark.mjs`.
...
`RegisteredModule.navigation` in `packages/contracts` is declared by modules but never read at run time (the shell reads `ModuleClientContribution.navigation`). `validateTaskPacket` and `packages/harness/schemas/task-packet.schema.json` have no runtime caller. `translations/*.json` have no loader. Extend the live path or remove the dead one in its own change; do not add a third variant.
