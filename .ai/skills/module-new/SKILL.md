...
- `pnpm oerp doctor --json` reports `status: healthy` (repository root only; the sandbox runs gates for you).
- `modules/<dir>/spec/module.yaml` exists, validates (`pnpm oerp spec validate --all --json`) and has `status: approved`. `pnpm oerp module new` refuses a draft with `A module can only be created from an approved specification.` In the sandbox the operator approves the spec after the business manager's turn, and the orchestrator runs the scaffold itself.
- Ids: module id and every permission id match `^[a-z][a-z0-9-]*(\.[a-z][a-z0-9-]*)+$`. `inventory.core` lives in `modules/inventory` as `@coreloom/module-inventory` (`packages/cli/src/module-scaffold.ts`, `packageSuffix`). Environment variables use the upper-case directory: `OERP_INVENTORY_DATABASE`.

## 2. Scaffold

```bash
pnpm oerp module new inventory.core --spec modules/inventory/spec/module.yaml          # dry run, lists files
pnpm oerp module new inventory.core --spec modules/inventory/spec/module.yaml --apply
```
...
`PlatformServerContext` also carries `settings: ModuleSettingsRuntime` and `agentTools: PlatformToolRegistry` (`modules/auth/src/server/composition.ts`, types from `@coreloom/kernel`). A composition may return `settings: defineModuleSettings({ moduleId: 'inventory.core', settings: { key: { type, defaultValue, visibility, client, scope?, label?, description?, min?, max?, enum?, secret? } } })` (rendered in Administration, Settings without more code; read live with `context.settings.get(tenantId, 'inventory.core', 'key')` at request time) and `start()` for work that needs every module composed (`platform/octane.config.ts` declares all settings, then calls every `start`). Agent tools register with `context.agentTools.register(...)` (`agent-tool-design`).
...
- Routes 404: `platform.server` missing, no `./platform` export, or `src/platform.ts` absent; `pnpm oerp module validate` names it (`PLATFORM_*`).
- `Kpi` typecheck error: `value` must be a string.
- `translations/*.json` are inert; `module.name` only, no UI copy.
