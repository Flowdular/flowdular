...
`PlatformServerContext` also carries `settings: ModuleSettingsRuntime`, `agentTools: PlatformToolRegistry`, `agentDefinitions: PlatformAgentRegistry`, and `capabilities: PlatformCapabilityRegistry` (`modules/auth/src/server/composition.ts`, types from `@coreloom/kernel`). A composition may return `settings: defineModuleSettings({ moduleId: 'inventory.core', settings: { key: { type, defaultValue, visibility, client, scope?, labelKey?, label?, descriptionKey?, description?, min?, max?, enum?, secret? } } })` (rendered in the module's drawer under Administration, Modules without more code; read live with `context.settings.get(tenantId, 'inventory.core', 'key')` at request time). Setting `labelKey` and `descriptionKey` values are fully qualified translation keys shipped by every module locale; keep English `label` and `description` literals as compatibility fallbacks and never translate values, identifiers or secrets. A composition may also return `start()` for work that needs every module composed and `dispose()` to release repositories, workers, timers and listeners (`platform/octane.config.ts` declares all settings, then calls every `start`, and owns every disposer). Agent tools register with `context.agentTools.register(...)` (`agent-tool-design`). Module-owned business agents register with `context.agentDefinitions.register(...)` after `defineAgent()` (`business-agent-design`). A module exposes or consumes a typed cross-module service with `context.capabilities.register/get/has`; the consumer declares the module dependency and handles an absent capability.
