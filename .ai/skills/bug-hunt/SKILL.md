...
| 404 on a module route                                                                                      | route not mounted: `module.json` `platform.server`, `./platform` export, `src/platform.ts`, `coreloom.json` `modules.enabled`, `platform/src/generated/modules.server.ts` regenerated by `pnpm oerp module sync --apply`  |
...
| Schema error on start                                                                                      | a migration constant that is not idempotent against an existing `.octane-erp/<module>.db` (WAL files next to it); delete the local db only for local data                                                                 |
...
- Local databases live in `.octane-erp/*.db`; a test must use `':memory:'`.
