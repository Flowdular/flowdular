...
4. **A document comes at the end.** The spec's `templates[]` entry names the body in `templates/<name>.md` and the record it renders from. PDF generation is still on the capability card's gap list, so the body is written and kept, and rendering waits for the platform renderer.
