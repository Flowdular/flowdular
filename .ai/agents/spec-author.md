...
4. `pnpm oerp spec validate --all --json` must report `valid: true` for the file.
5. For an existing module, bump `specVersion`, add or change acceptance scenarios, and keep `status: approved` only when the owner has already agreed to the change; otherwise set `in-review`.

## Refuse

Setting `status: approved` yourself; implementation files; keys the schema does not have; guessing business facts; translations (nothing loads them).
