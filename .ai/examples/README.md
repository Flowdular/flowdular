...
Small, deliberately isolated files that show a shape. None of them is compiled or tested; `.ai/references/catalog` is the compiled reference for a module and `modules/auth/src/cli` for a CLI extension.
...
- `bad/`: three negative examples, each with a README naming the violated `AGENTS.md` rule and the repair blueprint: `missing-acl` (raw route without access), `tenant-from-body` (tenant id from input, no session denial), `client-imports-server` (browser code importing the database repository).
