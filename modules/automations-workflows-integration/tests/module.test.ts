...
describe('automations-workflows.integration', () => {
	...
	it('registers exactly one workflow adapter and refuses duplicate composition', async () => {
	...
	it('stays optional when either public capability is absent', async () => {
		const capabilities = createPlatformCapabilityRegistry();
	...
	it('lists only the trusted tenant and validates the complete permission ceiling', async () => {
		const { capability } = workflowCapability();
...
describe('workflow runs an automation schedule', () => {
	...
	function tool(capability: AutomationExecutionCapability | null) {
	...
	it('refuses when automations is not composed', async () => {
		await expect(
