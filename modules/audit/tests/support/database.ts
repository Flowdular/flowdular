 ...
 */
export async function openAuditTestDatabase(): Promise<AuditTestDatabase> {
	...
	try {
		...
		leases.push(owner);
		await migrateAuditDatabase(owner.database);
		...
		return {
			...
			async reset() {
				await owner.database.transaction(
					(transaction) =>
						transaction.execute({
							text: `TRUNCATE ${AUDIT_TENANT_TABLES.join(', ')}`,
						}),
					{ access: 'write' },
				);
			},
