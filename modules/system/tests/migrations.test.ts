...
import { migrateSystemDatabase } from '../src/services/database-repository.ts';
import {
...
describe('system migrations', () => {
	...
	it('forces row level security with a tenant policy on every tenant table', async () => {
		const database = await migrator();
		await migrateSystemDatabase(database);

		for (const table of SYSTEM_TENANT_TABLES) {
	...
	it('refuses a flag outside 0 and 1 and a second row for the same module', async () => {
		const database = await migrator();
		await migrateSystemDatabase(database);
		const insert = (active: number, moduleId: string) =>
			database.transaction(
				(transaction) =>
					transaction.execute({
						text: `INSERT INTO system_module_activations
						 (tenant_id, module_id, active, changed_by, changed_at)
						 VALUES ('tenant-a', $1, $2, 'owner', 1)`,
						parameters: [moduleId, active],
					}),
				{ access: 'write', tenantId: 'tenant-a' },
			);

		await insert(0, 'reports.core');
		await expect(insert(2, 'metering.core')).rejects.toThrow();
		await expect(insert(1, 'reports.core')).rejects.toThrow();
	});
