...
export const SEARCH_TENANT_TABLES = ['search_recent_queries'] as const;

export interface SearchTestDatabase {
	/** Migrated provider, for a runtime that should acquire its own leases. */
	readonly databases: DatabaseProvider;
	readonly repository: DatabaseSearchRepository;
 ...
 */
export async function openSearchTestDatabase(): Promise<SearchTestDatabase> {
	...
	try {
		...
		leases.push(owner);
		await migrateSearchDatabase(owner.database);
		...
		return {
			databases,
			repository: new DatabaseSearchRepository(runtime.database),
			runtime: runtime.database,
			async reset() {
				await owner.database.transaction(
					(transaction) =>
						transaction.execute({
							text: `TRUNCATE ${SEARCH_TENANT_TABLES.join(', ')}`,
						}),
					{ access: 'write' },
				);
			},
