...
import { METERING_TENANT_TABLES } from '../../src/services/migration.ts';

export { METERING_TENANT_TABLES };

export interface MeteringTestDatabase {
	/** Migrated provider, for a runtime that should acquire its own leases. */
	readonly databases: DatabaseProvider;
	readonly repository: DatabaseMeteringRepository;
 ...
 */
export async function openMeteringTestDatabase(): Promise<MeteringTestDatabase> {
	...
	try {
		...
		leases.push(owner);
		await migrateMeteringDatabase(owner.database);
		...
		return {
			databases,
			repository: new DatabaseMeteringRepository(runtime.database),
			runtime: runtime.database,
			async reset() {
				await owner.database.transaction(
					(transaction) =>
						transaction.execute({
							text: `TRUNCATE ${METERING_TENANT_TABLES.join(', ')}`,
						}),
					{ access: 'write' },
				);
			},
