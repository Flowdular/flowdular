-- Documentation only; Flowdular never executes a down script. Reversing 0036
-- means taking the document template scopes back from every membership and
-- from the built-in role rows. It cannot distinguish a grant this migration
-- made from one a workspace made by assigning a role that names the scope.
ALTER TABLE auth_membership_scopes NO FORCE ROW LEVEL SECURITY;
DELETE FROM auth_membership_scopes
WHERE scope IN ('documents.templates.read', 'documents.templates.manage');
...
SET scopes_json = COALESCE(
      (SELECT json_agg(entry.scope ORDER BY entry.position)::text
        ...
        WHERE entry.scope NOT IN ('documents.templates.read', 'documents.templates.manage')),
      '[]')
