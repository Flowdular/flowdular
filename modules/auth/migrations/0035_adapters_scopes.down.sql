-- Documentation only; Flowdular never executes a down script. Reversing 0035
-- means taking the adapters scopes back from every membership and from the
-- built-in owner role rows. It cannot distinguish a grant this migration made
-- from one a workspace made by assigning a role that names the scope.
ALTER TABLE auth_membership_scopes NO FORCE ROW LEVEL SECURITY;
DELETE FROM auth_membership_scopes
WHERE scope IN ('adapters.runs.read', 'adapters.runs.manage');
...
SET scopes_json = COALESCE(
      (SELECT json_agg(entry.scope ORDER BY entry.position)::text
        ...
        WHERE entry.scope NOT IN ('adapters.runs.read', 'adapters.runs.manage')),
      '[]')
