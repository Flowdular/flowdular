...
/* Mirrors migrations/0034_research_scopes.up.sql byte for byte. */
export const AUTH_MIGRATION_034_RESEARCH_SCOPES = `-- research.core declares three permissions. Owners hold every permission an
-- enabled module declares, and members may read evidence and run research
-- through the workspace adapter and its budget, while research.settings.manage
-- stays with owners. The seed lists in acl/scopes.ts reach a workspace created
-- from now on; this grants the same defaults to the memberships and the built-in
-- role rows that already exist, whether or not auth sync-scopes reached them
...
SELECT account_id, tenant_id, scope
FROM auth_memberships
CROSS JOIN unnest(ARRAY['research.evidence.read', 'research.run', 'research.settings.manage']) AS granted(scope)
WHERE auth_memberships.role = 'owner'
ON CONFLICT DO NOTHING;
INSERT INTO auth_membership_scopes (account_id, tenant_id, scope)
SELECT account_id, tenant_id, scope
FROM auth_memberships
CROSS JOIN unnest(ARRAY['research.evidence.read', 'research.run']) AS granted(scope)
WHERE auth_memberships.role = 'member'
ON CONFLICT DO NOTHING;
ALTER TABLE auth_membership_scopes FORCE ROW LEVEL SECURITY;
ALTER TABLE auth_memberships FORCE ROW LEVEL SECURITY;
-- A role row grants too, and assigning a role replaces the membership scopes
-- with its list, so the built-in rows gain the same defaults: the scopes
...
SET scopes_json = (
      SELECT json_agg(entry.scope ORDER BY entry.origin, entry.position)::text
        FROM (
               ...
               UNION ALL
               SELECT 1 AS origin, added.position, added.scope
                 FROM unnest(ARRAY['research.evidence.read', 'research.run', 'research.settings.manage'])
                      WITH ORDINALITY AS added(scope, position)
                WHERE added.scope NOT IN (
                        SELECT existing.scope
                          FROM json_array_elements_text(auth_roles.scopes_json::json) AS existing(scope))
...
SET scopes_json = (
      SELECT json_agg(entry.scope ORDER BY entry.origin, entry.position)::text
        FROM (
               ...
               UNION ALL
               SELECT 1 AS origin, added.position, added.scope
                 FROM unnest(ARRAY['research.evidence.read', 'research.run'])
                      WITH ORDINALITY AS added(scope, position)
                WHERE added.scope NOT IN (
                        SELECT existing.scope
                          FROM json_array_elements_text(auth_roles.scopes_json::json) AS existing(scope))
...
/* Mirrors migrations/0035_adapters_scopes.up.sql byte for byte. */
export const AUTH_MIGRATION_035_ADAPTERS_SCOPES = `-- adapters.core declares two permissions and both stay with owners: an adapter
...
/* Mirrors migrations/0036_documents_templates_scopes.up.sql byte for byte. */
export const AUTH_MIGRATION_036_DOCUMENTS_TEMPLATES_SCOPES = `-- documents.core declares two template permissions. Owners hold both, and
-- members may read the workspace's document templates, while editing and
-- reverting them stays with owners. The seed lists in acl/scopes.ts reach a
-- workspace created from now on; this grants the same defaults to the
-- memberships and the built-in role rows that already exist, whether or not
-- auth sync-scopes reached them when the module was updated.
--
-- Row security is forced on all three tables and the migrator holds no tenant
-- setting, so the force flag is lifted on the source and the target for the
-- owner and put back inside the same transaction, as 0031 does.
...
SELECT account_id, tenant_id, scope
FROM auth_memberships
CROSS JOIN unnest(ARRAY['documents.templates.read', 'documents.templates.manage']) AS granted(scope)
WHERE auth_memberships.role = 'owner'
ON CONFLICT DO NOTHING;
INSERT INTO auth_membership_scopes (account_id, tenant_id, scope)
SELECT account_id, tenant_id, scope
FROM auth_memberships
CROSS JOIN unnest(ARRAY['documents.templates.read']) AS granted(scope)
WHERE auth_memberships.role = 'member'
ON CONFLICT DO NOTHING;
ALTER TABLE auth_membership_scopes FORCE ROW LEVEL SECURITY;
ALTER TABLE auth_memberships FORCE ROW LEVEL SECURITY;
...
SET scopes_json = (
      SELECT json_agg(entry.scope ORDER BY entry.origin, entry.position)::text
        FROM (
               ...
               UNION ALL
               SELECT 1 AS origin, added.position, added.scope
                 FROM unnest(ARRAY['documents.templates.read', 'documents.templates.manage'])
                      WITH ORDINALITY AS added(scope, position)
                WHERE added.scope NOT IN (
                        SELECT existing.scope
                          FROM json_array_elements_text(auth_roles.scopes_json::json) AS existing(scope))
...
SET scopes_json = (
      SELECT json_agg(entry.scope ORDER BY entry.origin, entry.position)::text
        FROM (
               ...
               UNION ALL
               SELECT 1 AS origin, added.position, added.scope
                 FROM unnest(ARRAY['documents.templates.read'])
                      WITH ORDINALITY AS added(scope, position)
                WHERE added.scope NOT IN (
                        SELECT existing.scope
                          FROM json_array_elements_text(auth_roles.scopes_json::json) AS existing(scope))
...
/* Mirrors migrations/0037_assistant_scopes.up.sql byte for byte. */
export const AUTH_MIGRATION_037_ASSISTANT_SCOPES = `-- agents.core declares agents.assistant.use, the permission a person needs to
...
export const databaseMigrations: readonly DatabaseMigration[] = [
	...
	/* Data-only like 0029, and for the same reason it declares no
	   inspectExisting. */
	{
		id: '0034_research_scopes',
		sql: { postgresql: AUTH_MIGRATION_034_RESEARCH_SCOPES },
	},
	/* Data-only like 0029. */
	{
		id: '0035_adapters_scopes',
		sql: { postgresql: AUTH_MIGRATION_035_ADAPTERS_SCOPES },
	},
	/* Data-only like 0029. */
	{
		id: '0036_documents_templates_scopes',
		sql: { postgresql: AUTH_MIGRATION_036_DOCUMENTS_TEMPLATES_SCOPES },
	},
	{
