...
describe('auth migrations', () => {
	...
	it('grants the enterprise module scopes to owner and member memberships by their defaults', async () => {
		...
		expect(seeded.scopes).toEqual([
			...
			{ account_id: 'account-ada', scope: 'adapters.runs.manage' },
			{ account_id: 'account-ada', scope: 'adapters.runs.read' },
			{ account_id: 'account-ada', scope: 'agents.assistant.use' },
			...
			{ account_id: 'account-ada', scope: 'research.evidence.read' },
			{ account_id: 'account-ada', scope: 'research.run' },
			{ account_id: 'account-ada', scope: 'research.settings.manage' },
			...
			{ account_id: 'account-bo', scope: 'agents.assistant.use' },
			...
			{ account_id: 'account-bo', scope: 'research.evidence.read' },
			{ account_id: 'account-bo', scope: 'research.run' },
	...
	/* research.core keeps research.settings.manage with owners and makes reading
	   evidence and running research member defaults, so 0034 grants both
	   memberships and both built-in role rows, each its own list. */
	it('AUTH-RESEARCH-SCOPES grants the research scopes to owners and members, in the memberships and the built-in role rows', async () => {
		const granted = databaseMigrations.findIndex(
			(migration) => migration.id === '0034_research_scopes',
		);
		expect(granted).toBeGreaterThan(0);
		await runDatabaseMigrations(
			lease.database,
		...
		await lease.database.transaction(
			async (transaction) => {
				...
				await transaction.execute({
					text: `INSERT INTO auth_membership_scopes (account_id, tenant_id, scope)
					       VALUES ('account-ada', 'tenant-a', 'research.run')`,
				});
				await transaction.execute({
					text: `INSERT INTO auth_roles
					       (id, tenant_id, key, name, description, scopes_json, builtin, created_at, updated_at)
					       VALUES ('tenant-a:owner', 'tenant-a', 'owner', 'Owner', 'Full access',
					               '["auth.profile.read","research.run"]', 1, 1, 1),
					              ('tenant-a:member', 'tenant-a', 'member', 'Member', 'Standard access',
					               '["auth.profile.read"]', 1, 1, 1),
					              ('tenant-a:analyst', 'tenant-a', 'analyst', 'Analyst', 'Custom',
		...
		const seeded = await lease.database.transaction(
			async (transaction) => ({
				scopes: (
					await transaction.query<{ account_id: string; scope: string }>({
						text: `SELECT account_id, scope FROM auth_membership_scopes
						       WHERE tenant_id = 'tenant-a' AND scope LIKE 'research.%'
						       ORDER BY account_id, scope`,
					})
				).rows,
				roles: (
	...
	/* adapters.core keeps both of its permissions with owners, so 0035 grants
	   the owner memberships and the built-in owner role row and nothing else. */
	it('AUTH-ADAPTERS-SCOPES grants the adapters scopes to owners alone, in the memberships and the built-in role rows', async () => {
		const granted = databaseMigrations.findIndex(
			(migration) => migration.id === '0035_adapters_scopes',
		);
		expect(granted).toBeGreaterThan(0);
		await runDatabaseMigrations(
			lease.database,
		...
		await lease.database.transaction(
			async (transaction) => {
				...
				await transaction.execute({
					text: `INSERT INTO auth_membership_scopes (account_id, tenant_id, scope)
					       VALUES ('account-ada', 'tenant-a', 'adapters.runs.read')`,
				});
				await transaction.execute({
					text: `INSERT INTO auth_roles
					       (id, tenant_id, key, name, description, scopes_json, builtin, created_at, updated_at)
					       VALUES ('tenant-a:owner', 'tenant-a', 'owner', 'Owner', 'Full access',
					               '["auth.profile.read","adapters.runs.read"]', 1, 1, 1),
					              ('tenant-a:member', 'tenant-a', 'member', 'Member', 'Standard access',
					               '["auth.profile.read"]', 1, 1, 1),
					              ('tenant-a:integrator', 'tenant-a', 'integrator', 'Integrator', 'Custom',
					               '["auth.profile.read"]', 0, 1, 1)`,
				});
			},
			{ tenantId: 'tenant-a', access: 'write' },
		...
		const seeded = await lease.database.transaction(
			async (transaction) => ({
				scopes: (
					await transaction.query<{ account_id: string; scope: string }>({
						text: `SELECT account_id, scope FROM auth_membership_scopes
						       WHERE tenant_id = 'tenant-a' AND scope LIKE 'adapters.%'
						       ORDER BY account_id, scope`,
					})
				).rows,
				roles: (
	...
	/* The workspace assistant acts as the member who asked, so 0037 grants its
	   one permission to owners and members alike. */
	it('AUTH-ASSISTANT-SCOPES grants the assistant scope to owners and members, in the memberships and the built-in role rows', async () => {
		const granted = databaseMigrations.findIndex(
			(migration) => migration.id === '0037_assistant_scopes',
		);
		expect(granted).toBeGreaterThan(0);
		await runDatabaseMigrations(
			lease.database,
		...
		await lease.database.transaction(
			async (transaction) => {
				...
				await transaction.execute({
					text: `INSERT INTO auth_membership_scopes (account_id, tenant_id, scope)
					       VALUES ('account-ada', 'tenant-a', 'agents.assistant.use')`,
				});
				await transaction.execute({
					text: `INSERT INTO auth_roles
					       (id, tenant_id, key, name, description, scopes_json, builtin, created_at, updated_at)
					       VALUES ('tenant-a:owner', 'tenant-a', 'owner', 'Owner', 'Full access',
					               '["auth.profile.read","agents.assistant.use"]', 1, 1, 1),
					              ('tenant-a:member', 'tenant-a', 'member', 'Member', 'Standard access',
					               '["auth.profile.read"]', 1, 1, 1),
					              ('tenant-a:editor', 'tenant-a', 'editor', 'Editor', 'Custom',
					               '["auth.profile.read"]', 0, 1, 1)`,
		...
		const seeded = await lease.database.transaction(
			async (transaction) => ({
				scopes: (
					await transaction.query<{ account_id: string; scope: string }>({
						text: `SELECT account_id, scope FROM auth_membership_scopes
						       WHERE tenant_id = 'tenant-a' AND scope = 'agents.assistant.use'
						       ORDER BY account_id`,
					})
				).rows,
				roles: (
