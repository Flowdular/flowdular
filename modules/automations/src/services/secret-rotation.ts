...
export const SECRET_ROTATION_TABLE = 'automations_triggers';
...
const SQL = {
	...
	/* The envelope the row still holds is the optimistic check: a secret rotated
	   ...
	   secret changes a caller can see, and a re-seal is not one. */
	reseal: `UPDATE automations_triggers
...
function count(value: number | bigint | string): number {
	...
	if (!Number.isSafeInteger(normalized)) {
		throw new Error('The automations database returned an invalid count.');
	}
	return normalized;
}

/**
 * Re-seals every stored trigger secret that is not on the current key. The
 * inventory is read once across tenants, and each batch of rows is read,
 ...
 */
export async function rotateTriggerSecrets(
