 ...
 */
export async function openAutomationsTestDatabase(): Promise<AutomationsTestDatabase> {
	...
	try {
		...
		leases.push(owner);
		await migrateAutomationsDatabase(owner.database);
		...
		return {
			databases,
			repository: new DatabaseAutomationsRepository({
			...
			async reset() {
				await owner.database.transaction(
					(transaction) =>
						transaction.execute({
							text: `TRUNCATE ${TENANT_TABLES.join(', ')}`,
						}),
					{ access: 'write' },
				);
			},
