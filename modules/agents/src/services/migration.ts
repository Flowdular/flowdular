...
export const AGENTS_MIGRATION_0026 = `-- The workspace assistant is a conversation of ordinary runs. A thread belongs
...
export const AGENTS_MIGRATION_0027 = `-- A decision provider is a provider connection whose kind answers typed
...
export const databaseMigrations: readonly DatabaseMigration[] = [
	...
	{
		id: '0026_assistant_threads',
		sql: { postgresql: AGENTS_MIGRATION_0026 },
		inspectExisting: async (database) => {
			const states = [
				await postgresTenantTableState(
					database,
					'assistant_threads',
					'assistant_threads_tenant_policy',
					[],
				),
				await postgresTenantTableState(
					database,
					'assistant_turns',
					'assistant_turns_tenant_policy',
					[],
				),
			];
			if (states.every((state) => state === 'complete')) return 'complete';
	...
	{
		id: '0027_agent_decision_providers',
		sql: { postgresql: AGENTS_MIGRATION_0027 },
		inspectExisting: (database) =>
			migrationObjectState([
				() =>
					database.schema.hasColumn(
