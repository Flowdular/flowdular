...
import { DIRECTORY_TENANT_TABLES } from '../../src/services/migration.ts';

export { DIRECTORY_TENANT_TABLES };

export interface DirectoryTestDatabase {
	/** Migrated provider, for a runtime that should acquire its own leases. */
	readonly databases: DatabaseProvider;
	readonly repository: DatabaseDirectoryRepository;
 ...
 */
export async function openDirectoryTestDatabase(): Promise<DirectoryTestDatabase> {
	...
	try {
		...
		leases.push(owner);
		await migrateDirectoryDatabase(owner.database);
		...
		return {
			databases,
			repository: new DatabaseDirectoryRepository(runtime.database),
			runtime: runtime.database,
			async reset() {
				await owner.database.transaction(
					(transaction) =>
						transaction.execute({
							text: `TRUNCATE ${DIRECTORY_TENANT_TABLES.join(', ')}`,
						}),
					{ access: 'write' },
				);
			},
