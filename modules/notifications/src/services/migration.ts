...
/* Mirrors migrations/0012_notifications_email_channel.up.sql byte for byte. */
export const NOTIFICATIONS_MIGRATION_012_EMAIL_CHANNEL = `-- A member asks for e-mail once, for the whole workspace, rather than per kind:
...
export const databaseMigrations: readonly DatabaseMigration[] = [
	...
	{
		id: '0012_notifications_email_channel',
		sql: { postgresql: NOTIFICATIONS_MIGRATION_012_EMAIL_CHANNEL },
		inspectExisting: (database) =>
			postgresTenantTableState(
				database,
				'notifications_member_preferences',
				'notifications_member_preferences_tenant_policy',
				[
					async () =>
						(
							await database.query<{ present: boolean }>({
							  ...
							  WHERE table_name = 'notifications_deliveries'
