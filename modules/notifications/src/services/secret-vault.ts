...
/** Bytes of a generated signing secret before base64url encoding. */
export const WEBHOOK_SECRET_BYTES = 32;
/** Characters of the SHA-256 an administrator compares after a rotation. */
export const SECRET_FINGERPRINT_LENGTH = 32;

/**
 * The additional data every webhook secret envelope is bound to. It is part of
 * the stored envelope: a rotation engine re-sealing a row under a new key must
 * rebuild exactly this string, so it lives with the envelope format.
 */
export function secretContext(
	tenantId: string,
	subscriptionId: string,
): string {
	return `${tenantId}:${subscriptionId}:notifications-webhook`;
}
...
/** Shown to administrators so a rotation is verifiable without the secret. */
export function secretFingerprint(secret: string): string {
	return createHash('sha256')
		.update(secret, 'utf8')
		.digest('hex')
		.slice(0, SECRET_FINGERPRINT_LENGTH);
}
...
function encryptionKey(
	value: string,
	variable = 'FD_NOTIFICATIONS_SECRET_KEY',
): Buffer {
...
export class AesGcmSecretVault implements SecretVault {
	...
	constructor(key: Buffer, previous: readonly Buffer[] = []) {
		if (key.byteLength !== 32) {
			throw new Error('Webhook secret encryption requires a 32-byte key.');
		}
	...
	encrypt(secret: string, context: string): EncryptedSecret {
		...
		if (normalized.length < 8 || normalized.length > 16_384) {
			throw new Error('Webhook secrets must contain 8 to 16384 characters.');
		}
	...
	decrypt(envelope: EncryptedSecret, context: string): string {
		try {
			...
			if (error instanceof KeyringError && error.code === 'KEY_UNKNOWN') {
				throw new Error('The webhook secret encryption key is unavailable.', {
					cause: error,
				});
			}
			throw error;
...
export function secretVaultFromEnvironment(
	...
	const previous = parsePreviousKeys(
		environment.FD_NOTIFICATIONS_SECRET_KEY_PREVIOUS,
		(entry) => encryptionKey(entry, 'FD_NOTIFICATIONS_SECRET_KEY_PREVIOUS'),
	);
	const configured = environment.FD_NOTIFICATIONS_SECRET_KEY;
	if (configured) {
		return new AesGcmSecretVault(encryptionKey(configured), previous);
	}
	if (environment.NODE_ENV === 'production') {
		throw new Error('FD_NOTIFICATIONS_SECRET_KEY is required in production.');
	}
	if (environment.NODE_ENV === 'test') {
		return new AesGcmSecretVault(Buffer.alloc(32, 0x4e), previous);
	}
	return new AesGcmSecretVault(
		developmentKey(
			flowdularLocalDataPath(workspaceRoot, 'notifications-secret.key'),
		),
		previous,
	);
}
