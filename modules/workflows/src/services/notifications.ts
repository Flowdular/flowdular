...
/* The notifications.core publish contract, declared here rather than imported.
   notifications.core is optional: workflows.core composes, runs and settles
   runs with that module absent, so it may not be a package dependency. This
   mirror is the contract notifications.core owns and must not drift from it.
   modules/agents/src/services/notifications.ts holds the same mirror and stays
   identical to this file apart from the module id; change both. */
...
/* Read at the point of use, never at composition time: the platform may
   register notifications.core after workflows.core, or never. */
...
export async function publishRunOutcome(
	...
	try {
		...
		console.warn(
			`[workflows] notification publish for run ${input.sourceRef} failed:`,
